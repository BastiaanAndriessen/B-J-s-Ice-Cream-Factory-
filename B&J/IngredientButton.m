...
- (id)initWithFrame:(CGRect)frame andIngredientDataObject:(Ingredient*)ingredientDataObject
{
    ...
    if (self) {
        // Initialization code
        //BtnBACK
        NSString *pathBg = [[NSBundle mainBundle] pathForResource:@"background_large" ofType:@"png" inDirectory:@"images/views/ingredients"];
        UIImage *Bg = [[UIImage alloc] initWithContentsOfFile:pathBg];
        [self setBackgroundImage:Bg forState:UIControlStateNormal];
        NSString *pathBgh = [[NSBundle mainBundle] pathForResource:@"background_largeh" ofType:@"png" inDirectory:@"images/views/ingredients"];
        UIImage *Bgh = [[UIImage alloc] initWithContentsOfFile:pathBgh];
        [self setBackgroundImage:Bgh forState:UIControlStateHighlighted];
        
        //self.backgroundColor = [UIColor colorWithRed:0.83f green:0.18f blue:0.44f alpha:1.00f];
