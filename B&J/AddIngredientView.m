...
#import "AddIngredientView.h"
#import "Constants.h"

@implementation AddIngredientView
...
@synthesize btnIceCream = _btnIceCream;
@synthesize btnChunks = _btnChunks;
@synthesize currentSelectedIngredientKind = _currentSelectedIngredientKind;

@synthesize addIngredientScrollV = _addIngredientScrollV;
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        // Initialization code
        self.currentSelectedIngredientKind = @"icecream";
        ...
        [self.btnBack addTarget:self action:@selector(backToTasteMixer:) forControlEvents:UIControlEventTouchUpInside];
        ...
        //BtnIceCream
        self.btnIceCream = [UIButton buttonWithType:UIButtonTypeCustom];
        self.btnIceCream.frame = CGRectMake(TopButtonLeftOffsetLeft, self.btnBack.frame.origin.y + self.btnBack.frame.size.height + 13, self.imageIngredient.size.width, self.imageIngredient.size.height);
        [self.btnIceCream setBackgroundImage:self.imageIngredient forState:UIControlStateNormal];
        [self.btnIceCream setBackgroundImage:self.imageIngredient forState:UIControlStateHighlighted];
        [self.btnIceCream setTitle:[@"Ice cream" uppercaseString] forState:UIControlStateNormal];
        [[self.btnIceCream titleLabel] setFont:[UIFont fontWithName:@"ChunkFive-Roman" size:TopButtonFontSize]];
        [self.btnIceCream setTitleColor:[UIColor colorWithRed:1.00f green:0.85f blue:0.27f alpha:1.00f] forState:UIControlStateNormal];
        [self addSubview:self.btnIceCream];
        
        //BtnChunks
        self.btnChunks = [UIButton buttonWithType:UIButtonTypeCustom];
        self.btnChunks.frame = CGRectMake(self.btnIceCream.frame.origin.x + self.btnIceCream.frame.size.width + 15, self.btnIceCream.frame.origin.y, self.imageIngredient.size.width, self.imageIngredient.size.height);
        [self.btnChunks setTitle:[@"chunks" uppercaseString] forState:UIControlStateNormal];
        [[self.btnChunks titleLabel] setFont:[UIFont fontWithName:@"ChunkFive-Roman" size:TopButtonFontSize]];
        [self.btnChunks setTitleColor:[UIColor colorWithRed:0.83f green:0.18f blue:0.44f alpha:1.00f] forState:UIControlStateNormal];
        [self.btnChunks setTitleColor:[UIColor colorWithRed:1.00f green:0.85f blue:0.27f alpha:1.00f] forState:UIControlStateHighlighted];
        [self addSubview:self.btnChunks];
        [self.btnChunks addTarget:self action:@selector(selectChunks:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)loadIngredients{
    ...
    self.mainView = [[UIView alloc] initWithFrame:CGRectMake(0, 192, [[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.height - 192)];
    [self addSubview:self.mainView];
    
    self.addIngredientScrollV = [[AddIngredientScrollView alloc] initWithFrame:CGRectMake(0, 0, self.mainView.frame.size.width, self.mainView.frame.size.height)];
    [self.addIngredientScrollV setUserInteractionEnabled:YES];
    [self.mainView addSubview:self.addIngredientScrollV];
    
    self.mainView.userInteractionEnabled = YES;
    self.addIngredientScrollV.scrollEnabled = YES;
    self.addIngredientScrollV.showsVerticalScrollIndicator = NO;
    self.addIngredientScrollV.delaysContentTouches = YES;
    self.addIngredientScrollV.canCancelContentTouches = NO;
    self.addIngredientScrollV.delegate = self.addIngredientScrollV;
    self.addIngredientScrollV.userInteractionEnabled = YES;
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(setContentSize:) name:@"SET_CONTENT_SIZE" object:nil];
}

-(void)selectIceCream:(id)sender{
    [self.btnIceCream setTitleColor:[UIColor colorWithRed:1.00f green:0.85f blue:0.27f alpha:1.00f] forState:UIControlStateNormal];
    [self.btnIceCream removeTarget:self action:@selector(selectIceCream:) forControlEvents:UIControlEventTouchUpInside];
    [self.btnIceCream setBackgroundImage:self.imageIngredient forState:UIControlStateNormal];
    [self.btnIceCream setBackgroundImage:self.imageIngredient forState:UIControlStateHighlighted];
    [self.btnChunks setBackgroundImage:nil forState:UIControlStateNormal];
    [self.btnChunks setBackgroundImage:nil forState:UIControlStateHighlighted];
    [self.btnChunks setTitleColor:[UIColor colorWithRed:0.83f green:0.18f blue:0.44f alpha:1.00f] forState:UIControlStateNormal];
    [self.btnChunks setTitleColor:[UIColor colorWithRed:1.00f green:0.85f blue:0.27f alpha:1.00f] forState:UIControlStateHighlighted];
    [self.btnChunks addTarget:self action:@selector(selectChunks:) forControlEvents:UIControlEventTouchUpInside];
    self.currentSelectedIngredientKind = @"icecream";
    [[NSNotificationCenter defaultCenter] postNotificationName:@"LOAD_INGREDIENTS" object:nil];
}

-(void)selectChunks:(id)sender{
    [self.btnChunks setTitleColor:[UIColor colorWithRed:1.00f green:0.85f blue:0.27f alpha:1.00f] forState:UIControlStateNormal];
    [self.btnChunks removeTarget:self action:@selector(selectChunks:) forControlEvents:UIControlEventTouchUpInside];
    [self.btnChunks setBackgroundImage:self.imageIngredient forState:UIControlStateNormal];
    [self.btnChunks setBackgroundImage:self.imageIngredient forState:UIControlStateHighlighted];
    [self.btnIceCream setBackgroundImage:nil forState:UIControlStateNormal];
    [self.btnIceCream setBackgroundImage:nil forState:UIControlStateHighlighted];
    [self.btnIceCream setTitleColor:[UIColor colorWithRed:0.83f green:0.18f blue:0.44f alpha:1.00f] forState:UIControlStateNormal];
    [self.btnIceCream setTitleColor:[UIColor colorWithRed:1.00f green:0.85f blue:0.27f alpha:1.00f] forState:UIControlStateHighlighted];
    [self.btnIceCream addTarget:self action:@selector(selectIceCream:) forControlEvents:UIControlEventTouchUpInside];
    self.currentSelectedIngredientKind = @"chunks";
    [[NSNotificationCenter defaultCenter] postNotificationName:@"LOAD_INGREDIENTS" object:nil];
}
...
-(void)setContentSize:(id)sender{
    self.addIngredientScrollV.contentSize = CGSizeMake([[UIScreen mainScreen] bounds].size.width, self.addIngredientScrollV.totalHeight);
}

-(void)backToTasteMixer:(id)sender{
