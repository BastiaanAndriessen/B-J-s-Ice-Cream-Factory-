...
#import "SettingsOverlayScrollView.h"
#import "Constants.h"

@implementation SettingsOverlayScrollView
...
@synthesize btnSaveSettings = _btnSaveSettings;
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.txtEmailAddress.text = [[[NSUserDefaults standardUserDefaults] objectForKey:@"email"] uppercaseString];
        ...
        //Btn Start Rating
        NSString *pathSaveBg = [[NSBundle mainBundle] pathForResource:@"default_wide_bottom" ofType:@"png" inDirectory:@"images/views/ratetastes/buttons"];
        UIImage *saveBg = [[UIImage alloc] initWithContentsOfFile:pathSaveBg];
        self.btnSaveSettings = [UIButton buttonWithType:UIButtonTypeCustom];
        self.btnSaveSettings.frame = CGRectMake([[UIScreen mainScreen] bounds].size.width/2 - saveBg.size.width/2, 335, saveBg.size.width, saveBg.size.height);
        [self.btnSaveSettings setBackgroundImage:saveBg forState:UIControlStateNormal];
        [self.btnSaveSettings setBackgroundImage:saveBg forState:UIControlStateHighlighted];
        [self.btnSaveSettings setTitle:[@"save settings" uppercaseString] forState:UIControlStateNormal];
        [[self.btnSaveSettings titleLabel] setFont:[UIFont fontWithName:@"ChunkFive-Roman" size:TopButtonFontSize]];
        [self.btnSaveSettings setTitleColor:[UIColor colorWithRed:1.00f green:0.86f blue:0.01f alpha:1.00f] forState:UIControlStateNormal];
        [self.btnSaveSettings setTitleColor:[UIColor colorWithRed:1.00f green:0.93f blue:0.53f alpha:1.00f] forState:UIControlStateHighlighted];
        [self addSubview:self.btnSaveSettings];
        [self.btnSaveSettings addTarget:self action:@selector(saveSettings:) forControlEvents:UIControlEventTouchUpInside];
    }
    return self;
}
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if(self.btnSaveSettings.frame.origin.y + self.btnSaveSettings.frame.size.height + 10 > [[UIScreen mainScreen] bounds].size.height && self.allowScrollControl){
        if(self.contentOffset.y > self.btnSaveSettings.frame.origin.y + self.btnSaveSettings.frame.size.height + 10 - [[UIScreen mainScreen] bounds].size.height){
            self.contentOffset = CGPointMake(0, self.btnSaveSettings.frame.origin.y + self.btnSaveSettings.frame.size.height + 10 - [[UIScreen mainScreen] bounds].size.height);
        }
    }
}
